...
namespace KinectWinforms
{
    ...
    public static class BezierSpline
    {
        ...
        public static void GetCurveControlPoints(Point[] knots, out Point[] firstControlPoints, out Point[] secondControlPoints)
        {
            ...
            {
                // First control point
                firstControlPoints[i] = new Point((int)(x[i] + 0.5), (int)(y[i] + 0.5));
                // Second control point
                if (i < n - 1)
                    secondControlPoints[i] = new Point((int)(2 * knots[i + 1].X - x[i + 1] + 0.5), (int)(2 * knots[i + 1].Y - y[i + 1] + 0.5));
                else
                    secondControlPoints[i] = new Point((int)((knots[n].X + x[n - 1]) / 2 + 0.5), (int)((knots[n].Y + y[n - 1]) / 2 + 0.5));
            }
        }
        ...
        /// Draw the Curve.
        /// </summary>
        public static void DrawCurve(Point[] points, out Point[] smoothedPoints)
        {
            smoothedPoints = new Point[points.Length];
            ...
            {
                smoothedPoints[index] = cp1[i];
                index++;
            }

           // // To draw a Bézier spline, you need an instance of the Graphics class and a Pen. 
           // // The instance of the Graphics class provides the DrawBezier method, and the Pen stores attributes, 
           // // such as width and color, of the line used to render the curve. 
           // // The Pen is passed as one of the arguments to the DrawBezier method. 
           // // The remaining arguments passed to the DrawBezier method are the endpoints and the control points. 
           // // The following example draws a Bézier spline with starting point (0, 0), control points (40, 20) and (80, 150), and ending point (100, 10):
