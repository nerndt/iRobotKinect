...
namespace iRobotKinect
{
    ...
    public class CSVReader : IDisposable
    {
        ...
        public List<object> ReadRow(string delimiter = ",")
        {
            ...
            while (currentLine != "")
                objects.Add(ReadNextObject(delimiter));
            return objects;
        }
        ...
        private object ReadNextObject(string delimiter = ",")
        {
            if (currentLine == null)
                return null;
            ...
            while (!foundEnd && i <= len)
            {
                ...
                if ((!quoted && i == len) // non-quoted strings end with a comma or end of line
                    || (!quoted && currentLine.Substring(i, 1) == delimiter) // || (!quoted && currentLine.Substring(i, 1) == ",")
                    // quoted strings end with a quote followed by a comma or end of line
                    || (quoted && i == len - 1 && currentLine.EndsWith("\""))
                    || (quoted && currentLine.Substring(i, 2) == delimiter2)) // || (quoted && currentLine.Substring(i, 2) == "\","))
                    foundEnd = true;
                else
                    i++;
            }
        ...
        public DataTable CreateDataTable(bool headerRow, string delimiter = ",")
        {
            // Read the CSV data into rows
            List<List<object>> rows = new List<List<object>>();
            List<object> readRow = null;
            while ((readRow = ReadRow(delimiter)) != null)
                rows.Add(readRow);
            ...
            {
                table.Columns.Add();
                if (i >= columnTypes.Count) // Assume string
                {
           ...
           // NGE09252013 for (int i = 0; i < columnTypes.Count; i++)
           // NGE09252013 {
           // NGE09252013     table.Columns.Add();
           // NGE09252013     table.Columns[i].DataType = columnTypes[i];
           // NGE09252013     if (i < columnNames.Count)
           // NGE09252013         table.Columns[i].ColumnName = columnNames[i];
           // NGE09252013 }
        ...
        public static DataTable ReadCSVFile(string filename, bool headerRow, int scanRows, string delimiter = ",")
        {
            using (CSVReader reader = new CSVReader(new FileInfo(filename)))
            {
                reader.ScanRows = scanRows;
                return reader.CreateDataTable(headerRow, delimiter);
            }
        }
